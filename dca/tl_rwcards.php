...
(
  ...
  class tl_rwcards extends Backend {
	...
	public function toggleVisibility($intId, $blnVisible)
	{
  	...
  	{
		$res = $this->Database->prepare("SELECT path FROM tl_files WHERE id = " . $arrRow['picture'])->execute();
		$this->picture = $res->fetchAllAssoc();
  		//print_r($this->picture[0]['path']);
		
		return '<div>
    <img src=" ' . $this->picture[0]['path'] . '  " style="height:100px; width:100px; float:left; margin-right: 1em;" /><p><strong>' . $arrRow['autor'] . '</strong> (' . $arrRow['email'] . ')</p>' . $arrRow['description']
  		. '</div>' . "\n";
  	}
