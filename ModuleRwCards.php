...
class ModuleRwCards extends Module {
    ...
    public function _getTemplate() {
        $this->import('Input');
        if ($this->Input->get('view') == "rwconecategory") {
    ...
    protected function compile() {
		...
		//$_SESSION['rwcards']['config'] == "";
		
        if ($_SESSION['rwcards']['config'] == "") {
        	$_SESSION['rwcards']['rwcards_session'] = session_id();
            $res = $this->Database->execute("select rwcards_cards_per_row, rwcards_rows_per_page, rwcards_keep_cards, rwcards_thumb_box_width, rwcards_thumb_box_height, rwcards_per_attachement,rwcards_thumbnail_width, rwcards_thumbnail_height from tl_module where type = 'RWCards'");
            $data = $res->fetchAssoc();
