...
class OneCategoryRwCards extends \Frontend
{
	...
	public function addToTemplate($objTemplate, $objConfig, $strSource, $intParent, $alias){
		...
		$resCats = $this->Database->prepare("select tl_rwcards.*, tl_files.path from tl_rwcards left join tl_files on tl_files.id =  tl_rwcards.picture  where tl_rwcards.pid = '" . $this->category_id . "' and tl_rwcards.published = '1' order by id");
		$objConfig->template = '';
		$objTemplate->template = 'rwcards_onecategory';
			...
			$resCats = $this->Database->prepare("select tl_rwcards.*, tl_files.path from tl_rwcards left join tl_files on tl_files.id =  tl_rwcards.picture  where tl_rwcards.pid = '" . $this->category_id . "' and tl_rwcards.published = '1' order by id");
			//echo "one cat: select tl_rwcards.*, tl_files.path from tl_rwcards  left join tl_files on tl_files.id =  tl_rwcards.picture where tl_rwcards.pid = '1' and tl_rwcards.published = '1'  order by id";
		}
