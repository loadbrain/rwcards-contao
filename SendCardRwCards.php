...
class SendCardRwCards extends Frontend {
	...
	public function addToTemplate($objTemplate, $objConfig, $strSource, $intParent, $alias) {
		...
		$lastId = array();
		$this->is_enduser = ($this->Input->get('sessionId') != "") ? 1 : 0;
		...
		$this->viewCardOnly = ($this->Input->get('view') == 'rwcardsReWriteCard' ) ? true : false;
		 ...
		 * set some vars
		 */
		$GLOBALS['TL_CSS'][''] = $this->Environment->base . "system/modules/rwcards/html/css/rwcards.previewandsend.css";
		...
		$objTemplate->reWritetoSender = ($this->Input->get('view') == "rwcardsReWriteCard") ? 1 : 0;
		$objTemplate->sessionId = ($this->Input->get('view') == "rwcardsReWriteCard") ? $this->Input->get('sessionId') : null;
		$objTemplate->alias = $alias;


		if ($this->Input->get('view') == "rwcardsReWriteCard") {
	...
	public function sendEcardPerMail($objTemplate) {
		...
		for ($i = 0; $i < count($this->lastId); $i++) {
			...
			$this->linkToRWCards[$i] = $this->Environment->base . $this->generateFrontendUrl('', $objTemplate->alias . '/view/rwcardsReWriteCard/sessionId/' . $_SESSION['rwcards']['rwcards_session'] . '/id/' . $this->lastId[$i] . '/read/1/sendmail/1' . '/' . $objTemplate->alias);
			$this->linkToViewOnly[$i] = $this->Environment->base . $this->generateFrontendUrl('', $objTemplate->alias . '/view/rwcardsReWriteCard/sessionId/' . $_SESSION['rwcards']['rwcards_session'] . '/id/' . $this->lastId[$i] . '/sendmail/0' . '/' . $objTemplate->alias);
			...
			$message[$i] = str_replace(array('[&]', '[lt]', '[gt]'), array('&', '<', '>'), $message[$i]);
	...
	public function setCardStatusToRead() {
		if ( $this->data[0]['cardRead'] == '0'){
			$res = $this->Database->prepare("update tl_rwcardsdata set cardRead = '1', readOn = '" . date("Y-m-d") . "' WHERE id = '" . $this->Input->get('id') . "' and sessionId = '" . $this->sessionId . "'")->execute();
