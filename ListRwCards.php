...
class ListRwCards extends Frontend
{
	...
	public function addCardsToTemplate($objTemplate, $objConfig, $strSource, $intParent, $alias){
		...
		//$objPage = $this->Database->prepare('SELECT * FROM tl_page WHERE id=28')->execute();
		...
		foreach ($this->categories as $val){
			...
			"tl_rwcards left join tl_rwcards_category on tl_rwcards_category.id = " . (int)$val['id'] . " left join tl_files on tl_files.id =  tl_rwcards.picture where (tl_rwcards.pid = " . (int)$val['id'] . " and tl_rwcards.published  = 1) " .
