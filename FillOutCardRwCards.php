...
class FillOutCardRwCards extends Frontend
{
	...
	global $objPage;
		...
		if ( $this->sessionId != ""){
			...
			$resCats = $this->Database->prepare("select tl_rwcardsdata.*, tl_rwcards.*, tl_files.path from tl_rwcardsdata, tl_rwcards left join tl_files on tl_files.id =  tl_rwcards.picture where tl_rwcards.id = '" . $this->card_id . "' and tl_rwcardsdata.sessionId = '" . $this->sessionId ."'");
			$this->data = $resCats->execute()->fetchAllAssoc();
		}
		else{
			$resCats = $this->Database->prepare("select tl_rwcards.*, tl_files.path from tl_rwcards left join tl_files on tl_files.id =  tl_rwcards.picture where tl_rwcards.id = '" . $this->card_id . "'"); 
			$this->data = $resCats->execute()->fetchAllAssoc();
		}
